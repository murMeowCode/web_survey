...
from main_auth.schemas.oauth import GoogleAuthRequest, OAuthUserResponse, TelegramAuthRequest
...
async def auth_via_telegram(
    ...
    oauth_data = await request_oauth_verification(
        nats, "telegram", request.model_dump()
    )

    if not oauth_data.get("success"):
        raise HTTPException(400, "Telegram OAuth failed")
